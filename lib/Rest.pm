...
require Exporter;
@ISA    = qw(Exporter);
@EXPORT = qw(set_rest_token rest_to_restserver members_of_maillist SFU_members_of_maillist roster_for_section info_for_computing_id);
...
sub members_of_maillist
{
    ...
    return undef if (!isMaillist($list));
...
sub SFU_members_of_maillist
{
    ...
    return undef if (!defined($members));
    foreach (@{$members})
    {
	push (@{$result},$_) if (!/\@/);
    }
...
{
    my $account = shift;
    my $result = rest_to_restserver("GET","/amaint/namespace.js?id=$account");
    return $result->{type};
}
...
{
    my $list = shift;
    return (get_account_type($list) eq "maillist");
}

sub existsInAmaint
{
    my $object = shift;
    my $result = get_account_type($object);
    return(defined($result) && $result ne "unknown");
}
